...
Solutions* T_solve(Torseur** list, int number) {
    int i, j, new_buff_size, count;
    Expression** e_final_list;
    Expression** e_list;
    Solutions* s;

    e_list = malloc(sizeof(Expression*) * 6);
    ...
    e_final_list = malloc(sizeof(Expression*) * new_buff_size);
    count = 0;
    for(i = 0; i < 6; i++) {
        if (!E_is_zero(e_list[i]))
    ...
    s = ES_solve(e_final_list, new_buff_size);
