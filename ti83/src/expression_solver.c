...
Solutions* ES_solve(Expression** expressions, int num_expressions) {
    ...
    num_elements = ES__remove_duplicates(buffer, num_vars, out_buffer);
    free(buffer);
    
    matrix = ES__malloc_matrix(num_elements, num_elements);
    value = malloc(num_elements * sizeof(double));
    solutions = malloc(num_elements * sizeof(double));
